...
Group_User.prototype = Object.extend(new LBase(), {
	...
	QryGroupData : function(){
		...
		this.ajaxCall(QryJson,"Admin.SystemMaintenance.Group_User","QryGroupData",GU.QryGroupDataHandler,false);
	},
	QryGroupDataHandler : function(ajax){
		if (xmlObject.readyState == 4 && xmlObject.status == 200) {
			...
			if(node==null||node.xml===undefined){
				node = GU.StrToXml(response.responseText);
			}
			//清空管理员组信息
			$("#group_list").jqGrid("clearGridData");
			var nodeJson = GU.XmlToJson(node);
	...
	QryAdminData : function(){
		...
		this.ajaxCall(QryJson,"Admin.SystemMaintenance.Group_User","QryAdminData",GU.QryAdminDataHandler,true);
	},
	...
	group_btn_delete : function(){
		...
		this.ajaxCall(QryJson,"Admin.SystemMaintenance.Group_User","DeleteGroupData",GU.group_btn_delete_handler,true);
	},
	...
	group_btn_save : function(){
		...
		this.ajaxCall(QryJson,"Admin.SystemMaintenance.Group_User","SaveGroupData",GU.group_btn_save_handler,true);
	},
...
//		});
		...
		this.ajaxCall(QryJson,"Admin.SystemMaintenance.Group_User","DeleteAdminData",GU.admin_btn_delete_handler,true);
	},
	...
	admin_btn_save : function(){
		...
		this.ajaxCall(QryJson,"Admin.SystemMaintenance.Group_User","SaveAdminData",GU.admin_btn_save_handler,true);
	},
